...
func AuthGetAcessTokenMultipleAccountsLast3days(c *gin.Context) {
	...
	config = &oauth2.Config{
		...
		RedirectURL:  "http://localhost:7070/oauth2callbackMultipleAccountstresdias", // Modifique conforme o necessário
		Scopes: []string{
...
func OAuth2CallbackMultipleAccountsLast3days(c *gin.Context) {
	...
	c.Redirect(http.StatusFound, "/MultipleAccountstresdias?access_token="+token.AccessToken)
}
func GetGoogleAdsDataForMultipleAccountsLast3days(c *gin.Context) {
	...
	// Pegar a data de 3 dias atrás
	threeDaysAgo := now.AddDate(0, 0, -3)

	// Formatar as datas como "YYYY-MM-DD"
	startDate := threeDaysAgo.Format("2006-01-02")
	...
	for _, customerID := range accountIDs {
		...
		for _, row := range res.Results {
			...
			allCampaigns = append(allCampaigns, models.AdsAccount{
				...
				Cost:        (float64(*row.Metrics.CostMicros) / 1e6) / 3,
	...
	err = sheets.WriteToGoogleSheetsLast3Days(ctx, accessToken, allCampaigns)
	if err != nil {
